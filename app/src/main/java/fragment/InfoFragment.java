...
import adapter.InfoFragmentAdapter;
...
public class InfoFragment extends BaseFragment implements OnClickListener,
	...
	private InfoFragmentAdapter ina;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		hGame = new CustomBean();
		ina = new InfoFragmentAdapter(getActivity());
			...
			case 0:
				...
				if (!Utils.isGoodJson(str)) {
					...
					if (customBean.getData().equals("")) {
						...
						java.lang.reflect.Type type1 = new TypeToken<CustomBean>() {
						}.getType();
						hGame = gson.fromJson(str, type1);
						for (int i = 0; i < hGame.getData().size(); i++) {
							if (!hGame.getData().get(i).getH_g_n_off()
									.equals("0")) {// 1显示 0隐藏
								HXGameNameBean hgn = new HXGameNameBean();
								hgn.setH_g_n_cid(hGame.getData().get(i)
										.getH_g_n_id());
