...
public class AutoNextLineView extends ViewGroup {
	private int mCellWidth;
	private int mCellHeight;
	private Context context;

	public AutoNextLineView(Context context) {
		super(context);
		this.context = context;
	}

	public AutoNextLineView(Context context, AttributeSet attrs) {
		super(context, attrs);
		this.context = context;
	}

	public AutoNextLineView(Context context, AttributeSet attrs, int defStyle) {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		setMeasuredDimension(resolveSize(mCellWidth * count, widthMeasureSpec),
				resolveSize(Utils.dip2px(context, 80), heightMeasureSpec));
